...
france_official_sidep_sg_iris_latest <- france_official_sidep_sg_iris[ date == "2020-10-06", ]
```
...
```{r}
my_data <- iris_sp_latest[ iris_sp_latest$REG %in% c(84) | TRUE,]
g <- ggplot(data = my_data)
g <- g + geom_sf(aes(fill = ti_class_mean), lwd = 0.1) +
