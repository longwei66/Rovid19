...
plot_france_line <- function(
  ...
  message("===== plot map ======")
  jpeg(filename = paste0(plot_date,"_la_france_covid.jpg"), width = 3000, height = 3000, units = "px")
  opar <- par(mar = c(0,0,0,0))
  plot(
    st_geometry(my_data)
    , col="lightblue3"
...
ffmpeg -framerate 1 -i %02d.jpg -c:v libx264 france_covid.mp4

loop and reverse the video 
70 = 22s of video lenght x 2 x 5 times
ffmpeg -i france_covid.mp4 -filter_complex "[0]reverse[r];[0][r]concat,loop=5:220,setpts=N/1/TB" output.mp4

accelerate
ffmpeg -i output.mp4 -filter:v fps=fps=12 output_final.mp4
...
```{r}
iris_sp_c <- st_centroid(iris_sp)
pts <- do.call(rbind, st_geometry(iris_sp_c))
pts <- as.data.table(pts)
pts$CODE_IRIS <- iris_sp_c$CODE_IRIS
pts$DEP <- iris_sp_c$DEP
```
...
my_data <- iris_sp_latest[ iris_sp_latest$DEP %in% c(
  "01","19","07","26","87","23","63","15","38","42","43",
  "69"
),]
final <- merge(x = pts, y = my_data, by.x = "CODE_IRIS", by.y = "CODE_IRIS", all.x = FALSE)
final$X <- final$V1
final$Y <- final$V2
...
g #+ stat_density_2d(data = final, aes(X, Y))
...
unique(france_official_sidep_sg_iris$date)



my_data <- iris_sp_latest[ iris_sp_latest$DEP %in% c("19"),]
final <- merge(x = pts, y = my_data, by.x = "CODE_IRIS", by.y = "CODE_IRIS", all.x = FALSE)
final$X <- final$V1
final$Y <- final$V2
...
bb <- st_bbox(final$geometry)
plot(st_geometry(final$geometry), col="ivory1", border = NA)
linemap(
  x = final, 
  var = "pop", 
  k = 2.5, 
