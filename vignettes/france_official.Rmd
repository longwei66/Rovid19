...
DT::datatable(tail(france_official_hospital_dep,200)
...
DT::datatable(tail(france_official_reg,100)
              #, extensions = 'FixedColumns'
              , filter = 'top'
              , class = 'cell-border stripe'
              , options = list(
...
```{r getSIDEPtot}
france_official_sidep_total <- data.table::as.data.table(read.csv(file = france_official_sidep_total_url, sep = ";"))
...
#my_df[ my_df$country == "France" & my_df$date == as.Date("2020-03-15"),]$cases <-   
...
```{r}
r_effective_france <- plot_R(my_country = "France", start_date = r0_date, last_date = last_date, mydata = france_com_cases_daily)
