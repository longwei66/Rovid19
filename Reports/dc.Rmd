...
### Insee code mapping (Ilde de France)

We have only mapping for Ile de France, need to look for whole France.

```{r}
insee_mapping <- as.data.table(read.csv(file = france_insee_zip_url, sep = ";", stringsAsFactors = FALSE))
```

```{r}
insee_mapping <- insee_mapping[ , .(
...
```{r}
dcd_by_code <- copy(dcd_all_clean)
dcd_by_code <- dcd_by_code[ , .(count = .N), by = .(code_lieu_deces,date_deces) ]
# convert to date
...
#### Visualisation for Paris

```{r}
dcd_by_code_s <- dcd_by_code[ year > 1972 ]
dcd_by_code_s <- dcd_by_code_s[ , .( sum_c = sum(count) ) , by = .(year,common_date)] 
...
```{r}
library(gghighlight)
g <- ggplot(data = dcd_by_code_s, group = year)
g <- g + geom_line(mapping = aes(x = common_date, y = sum_c, col = as.factor(year)), size = 0.5)
g <- g + gghighlight(year %in% c(2020,2003,1989, 1994, 1997,1970))
...
communes_results <- merge(
  x = dcd_by_code[ date_deces == "2020-04-01"]
