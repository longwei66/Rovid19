...
france_geojson <- "https://github.com/gregoiredavid/france-geojson/raw/master/communes.geojson"
...
```{r simplifyShapeFile}
communes_lim <- rmapshaper::ms_simplify(communes_lim, keep = 0.05)
```
...
### France Geojson second approach

```{r}
france_communes_g <- geojsonio::geojson_read(france_geojson,  what = "sp")
```

This dataset contains communes (county ?) shapefiles as well as some metadata

```{r exampleFranceShapeData}
head(france_communes_g@data) %>%
    kable() %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```
...
```{r simplifyShapeFile}
communes_lim_g <- rmapshaper::ms_simplify(france_communes_g, keep = 0.05)
```
...
# communes_lim_fortified <- broom::tidy(communes_lim, region = "insee")
# broom tidy is hanging
communes_lim_fortified_g <- fortify(france_communes_g, region = "code")
...
# Now I can plot this shape easily as described before:
ggplot() +
  geom_polygon(data = communes_lim_fortified_g, 
               aes( x = long, y = lat, group = group),
               fill="white", 
               color="grey", size = 0.2
               ) +
...
p <- ggplot() +
  geom_polygon(
    data = communes_results_w[ departement_insee_code %in% c("92","91","93", "94","95","78", "69", "13","33","19")], 
    aes(fill = count, x = long, y = lat, group = group),
    color="grey", size = 0.02
  ) +
  scale_fill_viridis_c(option = "A", direction = -1) +
