...
# france_population_per_departement
load(
  file = paste0(
    data_storage_url
...
In this section we will create some aggregated data at various levels.

## France Total deaths per day

### Prepare the data

We first count the number of deaths per day for the whole country.
...
```{r mortalityComparison, echo=FALSE}
g <- ggplot(
...
## France deaths per deparement per day

### Prepare the data

As a second step, we compute the daily number of deaths per location code, at 
communes level.
...
We merge the resulting data with the mapping between communes codes and 
departements codes.
...
Finally we aggregate the data at departement level, summing all communes' data
included in each departement.
...
dcd_by_department_average <- dcd_by_department_average[ , .(deaths_av  = mean(daily_dep_deaths)), by = .(departement_insee_code,common_date_death) ]
dcd_by_department_average <- dcd_by_department_average[order(departement_insee_code,common_date_death)]
```
...
g <- g + geom_line(aes(x= common_date_death, y = deaths_av_smooth), col = "darkred", alpha = 0.8)
# Add 2020 data
#g <- g + geom_line(data = my_data_2020 , aes( x= common_date_death, y = daily_dep_deaths)
...
## France Mortality incidence (mortality rate)
