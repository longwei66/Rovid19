...
```{r}
last_date_fr <- ymd("2020-04-02")
```


## Get Data from Initial source
...
## Get data from community alternative source
```{r}
...
```{r}
total_fr_daily <- getDailyRegion(region = "Total", df = fr_cum_long)
total_fr_daily %>%  ggplot(aes(x=Date, y=incident_cases)) +
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
