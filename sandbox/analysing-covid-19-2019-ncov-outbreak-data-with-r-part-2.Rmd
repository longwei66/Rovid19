...
version <- "1.3"
version_date <- lubridate::ymd("2020-03-06")
...
This post is a modified version of the initial code by Tim Churches, France was
added with a not so reliable source :

Italy code was hacked to cope with recent data on wikipedia.
Nevertheless, we should use now a better source :
...
#### Get the data from opendatasoft.com

```{r}
#  Get data for France
src_france <- "https://data.opendatasoft.com/explore/dataset/evolution-de-la-pandemie-de-covid-19-en-france@public/download/?format=csv&timezone=Europe/Berlin&use_labels_for_header=true&csv_separator=%3B"
france <- read.csv(file = src_france, sep = ";")

head(france)

```

#### Reshape the data to fit later stages

```{r}
fr_cum0 <- france %>%
  rename(
    Region = "Nom.officiel"
    , cumulative_cases = "Nombre.de.cas.confirmés"
    , country_region = "Code.ISO.3166"
  ) %>%
  select(-Code.officiel, - Type.de.zone, -Centroide, -Source, -URL) %>%
  mutate(
...
#### Add Total for France (a new region)

```{r get_france_data, message=FALSE, warning=FALSE, tidy=TRUE}
fr_cum0 %>%
  group_by(Date) %>%
  summarise(cumulative_cases=sum(cumulative_cases, na.rm=TRUE)) %>%
  ungroup() %>%
  mutate(  country_region = "France", Region = "Total") %>%
  select(country_region, Region, Date, cumulative_cases )-> fr_cum0_t

fr_cum0 <- rbind(fr_cum0, fr_cum0_t)
...
fr_cum0 %>%
  ...
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
       title="COVID-19 incident cases in France (source 2)") +
  theme(legend.position="top")
```


#### Let's have a look to cumulative_cases per region

Warning, Y scales are different for each plot.
...
fr_cum0 %>% 
  ...
  ggplot(aes(x=Date, y=cumulative_cases)) + 
  geom_bar(stat="identity", fill="green") + 
  facet_wrap(Region ~., ncol=3, scales = "free_y") + labs(y="Cumulative Cases - France per region",
                                       title="Confirmed cases of COVID-19 in french regions",
                                       subtitle="Note: differing y-axis scales") +
  theme(legend.position = "none", 
        strip.text.y = element_text(size=12)) -> fr
print(fr)
```
...
#### Compute Incident cases (daily new cases)

```{r}
getDailyRegion <- function(region = NULL, df){
  region_daily <- df %>%
...
Test the function for "Ile-de-France"

```{r}
total_fr_daily1 <- getDailyRegion(region = "Ile-de-France", df = fr_cum0)
total_fr_daily1 %>%  ggplot(aes(x=Date, y=incident_cases)) +
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
       title="COVID-19 incident cases in Ile de France (source 1)") +
  theme(legend.position="top")
```


Test the function for "Total"

```{r}
total_fr_daily1 <- getDailyRegion(region = "Total", df = fr_cum0)
total_fr_daily1 %>%  ggplot(aes(x=Date, y=incident_cases)) +
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
       title="COVID-19 incident cases in France (source 1)") +
  theme(legend.position="top")
```

Now we compute for all regions
```{r}
fr_daily0 <- lapply(unique(fr_cum0$Region), getDailyRegion, df = fr_cum0)
french_regions_confirmed0 <- data.table::rbindlist(fr_daily0)
```
...
```{r, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
french_regions_confirmed0 %>% 
  group_by(Region) %>%
  ggplot(aes(x=Date, y=incident_cases)) + 
  geom_bar(stat="identity", fill="green") + 
  facet_wrap(Region ~., ncol=3, scales = "free_y") + labs(y="Incident Cases - France per region",
                                       title="Confirmed cases of COVID-19 in french regions",
                                       subtitle="Note: differing y-axis scales") +
  theme(legend.position = "none", 
        strip.text.y = element_text(size=12)) -> fr
...
plot_R_fr <- function(region, region_start_date, mydata = french_regions_confirmed0) {
  
  confirmed_cases_fr <- mydata %>%
    filter(Date <= last_date_fr) %>% 
  ...
  estimate_R_obj <- estimate_R(confirmed_cases_fr,
                               method="uncertain_si",
                               config = make_config(list(
                                 mean_si = 7.5, std_mean_si = 2.0,
                                 min_mean_si = 1, max_mean_si = 8.4,
...
epicurve_fr <- function(region, mydata = french_regions_confirmed0) {
  mydata %>%
    filter(Region == region) %>%
    group_by(Date) %>%
    summarise(incident_cases=sum(incident_cases, na.rm=TRUE)) %>%
...
lambda_plot_fr <- function(region, mydata = french_regions_confirmed0) {
...
epicurve_fr(region,mydata = french_regions_confirmed0[ Region == "Total"])
```

```{r}
lambda_plot_fr(region, mydata = french_regions_confirmed0[ Region == "Total"])
```

```{r}
plot_R_fr(region, region_start_date, mydata = french_regions_confirmed0[ Region == "Total"])
```

**Assessment**: The outbreak is not yet under control,  $R_{e}$ is consistently falling but still high (above 3).
...
region_start_date <- ymd("2020-02-26")

epicurve_fr(region, mydata = french_regions_confirmed0[ Region == "Ile-de-France"])
```

```{r}
lambda_plot_fr(region,  mydata = french_regions_confirmed0[ Region == "Ile-de-France"])
```

```{r}
plot_R_fr(region, region_start_date,  mydata = french_regions_confirmed0[ Region == "Ile-de-France"])
```
...
epicurve_fr(region, mydata = french_regions_confirmed0[ Region == "Grand Est"])
```

```{r}
lambda_plot_fr(region,  mydata = french_regions_confirmed0[ Region == "Grand Est"])
```

```{r}
plot_R_fr(region, region_start_date,  mydata = french_regions_confirmed0[ Region == "Grand Est"])
```
...
Let's get the cum data by Region from a github repo (to be checked)

```{r}
git_fr_url <- "https://github.com/cedricguadalupe/FRANCE-COVID-19/raw/master/france_coronavirus_time_series-confirmed.csv"
fr_cum1 <- read_csv(git_fr_url)

# We have missing data in some column, we will by latest known Value
fr_cum1 <- fr_cum1 %>% fill(`Auvergne-Rhône-Alpes`)
fr_cum1 <- fr_cum1  %>% fill(`Bourgogne-Franche-Comté`)
fr_cum1 <- fr_cum1  %>% fill(`Île-de-France`)
fr_cum1 <- fr_cum1  %>% fill(Occitanie)
fr_cum1 <- fr_cum1  %>% fill(`Pays de la Loire`)
fr_cum1 <- fr_cum1  %>% fill(`Provence-Alpes-Côte d'Azur`)
complete.cases(fr_cum1)
```

Some data are incorrect for Ile de France, let's fix.
https://www.iledefrance.ars.sante.fr/coronavirus-covid-19-points-de-situation

```{r ars_fix}
fr_cum1 <- as.data.table(fr_cum1)
fr_cum1[ Date == "05/03/2020", `Île-de-France` := 91 ]
fr_cum1[ Date == "06/03/2020", `Île-de-France` := 148 ]
fr_cum1[ Date == "07/03/2020", `Île-de-France` := 178 ]
fr_cum1[ Date == "09/03/2020", `Île-de-France` := 300 ]
#fr_cum[ Date == "09/03/2020", Total := 1412 ]
...
fr_cum_long1 <- fr_cum1 %>%
  pivot_longer(-Date) %>%
  rename(cumulative_cases = "value", Region = "name") %>%
  mutate(Date = lubridate::dmy(Date), country_region = "France", incident_cases = 0) %>%
...
## Alternative source
```{r}
src <- "https://raw.githubusercontent.com/opencovid19-fr/data/master/dist/chiffres-cles.csv"
dt <- data.table(read.csv(file = src))
head(dt)
```
...
dt %>% 
  ...
  rename(
    Date = date
    , Region = maille_nom
    , cumulative_cases = cas_confirmes
  ) %>%
  mutate(
    incident_cases = 0,
    Date = as.Date(Date)
  ...
  select(
    -granularite, -maille_code, -deces, -source_nom, -source_url
  )-> fr_cum_long
head(fr_cum_long)
```
...
Now we compute the daily cases
...
```{r}
total_fr_daily1 <- getDailyRegion(region = "Total", df = fr_cum_long1)
total_fr_daily1 %>%  ggplot(aes(x=Date, y=incident_cases)) +
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
       title="COVID-19 incident cases in France (source 1)") +
...
Now we compute for all regions
```{r}
fr_daily1 <- lapply(unique(fr_cum_long1$Region), getDailyRegion, df = fr_cum_long1)
french_regions_confirmed1 <- data.table::rbindlist(fr_daily1)
```

```{r}
fr_daily <- lapply(unique(fr_cum_long$Region), getDailyRegion, df = fr_cum_long)
french_regions_confirmed <- data.table::rbindlist(fr_daily)
...
This does not match with total data, we assume that the dataset is not really clean (Total is not the sum of regions)
...
french_regions_confirmed1 %>%
  ...
  labs(y="Total cases to date",
       x = "",
       title="COVID-19 total cases in France, by region - Source 2") +
  theme(legend.position="top")
```
...
french_regions_confirmed1 %>% 
  ...
  summarise(total=sum(incident_cases, na.rm=TRUE)) %>%
  arrange(desc(total)) %>%
  pull(Region) -> french_regions_order

fr <- french_regions_confirmed1 %>%
  filter(Date <= last_date_fr,
         Region %in% french_regions_order) %>%
  mutate(Region=ordered(Region, levels=french_regions_order)) %>%
  ggplot(aes(x=Date, y=incident_cases)) + 
  geom_bar(stat="identity", fill="green") + 
  facet_wrap(Region ~., ncol=3) + labs(y="Daily incremental incidence",
                                       title="Confirmed cases of COVID-19 in french regions",
                                       subtitle="Note: differing y-axis scales") +
  theme(legend.position = "none", 
        strip.text.y = element_text(size=12))
...
```{r Cori_model_get_data_fr, echo=FALSE, tidy=TRUE, message=FALSE, eval=TRUE}
plot_R_fr <- function(region, region_start_date, mydata = french_regions_confirmed1) {
...
epicurve_fr <- function(region, mydata = french_regions_confirmed1) {
  mydata %>%
    filter(Region == region) %>%
    group_by(Date) %>%
    summarise(incident_cases=sum(incident_cases, na.rm=TRUE)) %>%
...
lambda_plot_fr <- function(region, mydata = french_regions_confirmed1) {
  res_obj <- prefectural_earlyR(mydata, 
                                region, 
                                last_date_fr,
                                si_mean,
...
```{r}
region <- "Total"
region_start_date <- ymd("2020-02-25")

epicurve_fr(region,mydata = total_fr_daily1)
```

```{r}
lambda_plot_fr(region, mydata = total_fr_daily1)
```

```{r}
plot_R_fr(region, region_start_date, mydata = total_fr_daily1)
```
...
```{r}
lambda_plot_fr(region)
```

```{r}
plot_R_fr(region, region_start_date)
```
...
But we have a doubt on the source data.
...
```{r}
italy <- data.table::as.data.table(italy)
italy <- italy %>%
  rename(
    Date = data
    , Region = denominazione_regione
    , incident_cases = nuovi_attualmente_positivi
    , cumulative_cases = totale_casi) %>%
  mutate(Date = as.Date(Date)) %>%
  mutate(country_region = "Italy") %>%
  select(country_region, Date, Region, incident_cases, cumulative_cases)
head(italy)
```
...
g <- g + facet_wrap(facets = . ~ Region, scales = "free_y")
g + ggtitle(label = "daily new cases")
g <- ggplot(data = italy)
g <- g + geom_bar(aes(x= Date, y = cumulative_cases), stat = "identity")
g <- g + facet_wrap(facets = . ~ Region, scales = "free_y")
g + ggtitle("Total cases")

```


### Add Total for Italy (a new region)
...
italy %>%
  ...
  ungroup() %>%
  mutate(  country_region = "Italy", Region = "Total") %>%
  select(country_region, Region, Date, incident_cases, cumulative_cases )-> italy_t

italy <- rbind(italy, italy_t)
italy <- data.table::as.data.table(italy)
...
italy %>%
  ...
  geom_bar(stat="identity") +
  labs(y="Incident cases", 
       title="COVID-19 incident cases in Italy (source 2)") +
  theme(legend.position="top")
```
...
```{r Cori_model_get_data_it2, echo=FALSE, tidy=TRUE, message=FALSE, eval=TRUE}
plot_R_it <- function(region, region_start_date) {
  
  confirmed_cases_it <- italy %>%
...
lambda_plot_it <- function(region) {
  res_obj <- prefectural_earlyR(italy, 
                                region, 
                                last_date_it,
                                si_mean,
                                si_sd)
...
epicurve_fr(region,mydata = italy[ Region == "Total"])
```

```{r}
lambda_plot_fr(region, mydata = italy[ Region == "Total"])
```

```{r}
plot_R_fr(region, region_start_date, mydata = italy[ Region == "Total"])
```
...
```{r get_italy_data_new1, message=FALSE, warning=FALSE, tidy=TRUE}

last_date_it <- ymd("2020-03-23")
...
italian_colnames <- italian_colnames[!italian_colnames %in% c("Centre", "New", "Total", "Sources") & !is.na(italian_colnames)]

italian_regions_confirmed <- italian_regions_confirmed[
  2:nrow(italian_regions_confirmed),
  1:length(italian_colnames)
  ]

colnames(italian_regions_confirmed) <- italian_colnames
```

```{r get_italy_data_clean, message=FALSE, warning=FALSE, tidy=TRUE}
# utility function to remove commas in numbers as character strings
rm_commas <- function(x) gsub(",", "", x)
rm_refs <- function(x) stringr::str_split(x, "\\[", simplify=TRUE)[,1]
#rm_parens <- function(x) stringr::str_split(x, "\\(", simplify=TRUE)[,1]
rm_parens <- function(x) trimws(gsub(pattern = "\\(.*\\)(.*)", replacement = "\\1" , x = x))
italian_regions_confirmed <- italian_regions_confirmed %>%
  mutate_if(is.character, rm_commas) %>%
  mutate_if(is.character, rm_refs) %>%
  mutate(Date=ymd(Date)) %>%
...
**Assessment**: The outbreak is not yet under control, but $R_{e}$ is consistently falling but still high (above 2.5).
...
**Assessment**: The outbreak is not yet under control, but $R_{e}$ is consistently falling.
