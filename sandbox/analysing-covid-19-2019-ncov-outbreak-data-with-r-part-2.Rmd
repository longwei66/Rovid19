...
fr_cum <- read_csv2(git_fr_url)

# We have missing data in some column, we will by latest known Value
fr_cum <- fr_cum %>% fill(`Auvergne-Rhône-Alpes`)
fr_cum <- fr_cum  %>% fill(`Bourgogne-Franche-Comté`)
fr_cum <- fr_cum  %>% fill(`Île-de-France`)
fr_cum <- fr_cum  %>% fill(Occitanie)
fr_cum <- fr_cum  %>% fill(`Pays de la Loire`)
fr_cum <- fr_cum  %>% fill(`Provence-Alpes-Côte d'Azur`)
complete.cases(fr_cum)
```
...
```{r ars_fix}
fr_cum <- as.data.table(fr_cum)
fr_cum[ Date == "05/03/2020", `Île-de-France` := 91 ]
fr_cum[ Date == "06/03/2020", `Île-de-France` := 148 ]
fr_cum[ Date == "07/03/2020", `Île-de-France` := 178 ]
fr_cum[ Date == "09/03/2020", `Île-de-France` := 300 ]
...
total_fr_daily %>%  ggplot(aes(x=Date, y=incident_cases)) +
    geom_bar(stat="identity") +
    labs(y="Incident cases", 
         title="COVID-19 incident cases in France") +
    theme(legend.position="top")
```
...
french_regions_confirmed %>%
    ...
    labs(y="Total cases to date",
         x = "",
         title="COVID-19 total cases in France, by region") +
    theme(legend.position="top")
```
...
plot_R_fr <- function(region, region_start_date) {
    
    confirmed_cases_fr <- french_regions_confirmed %>%
        filter(Date <= last_date_fr) %>% 
...
lambda_plot_fr <- function(region) {
    res_obj <- prefectural_earlyR(french_regions_confirmed, 
                                  region, 
                                  last_date_fr,
                                  si_mean,
                                  si_sd)
