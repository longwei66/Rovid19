...
last_date_fr <- ymd("2020-03-17")


french_regions_confirmed %>%
  filter(Region != "Total") %>%
  group_by(Date) %>%
  summarise(incident_cases=sum(incident_cases, na.rm=TRUE)) %>%
...
plot_R_fr <- function(region, region_start_date, mydata = french_regions_confirmed) {
...
epicurve_fr <- function(region, mydata = french_regions_confirmed) {
  mydata %>%
    filter(Region == region) %>%
    group_by(Date) %>%
    summarise(incident_cases=sum(incident_cases, na.rm=TRUE)) %>%
...
lambda_plot_fr <- function(region, mydata = french_regions_confirmed) {
